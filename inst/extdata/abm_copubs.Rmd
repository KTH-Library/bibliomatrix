...
### INCLUDE THIS IN PUBLIC DATA AND ADD DATA RETRIEVAL TO LOGIC BELOW
con <- pool_bib()
df_copub_countries <- abm_copub_countries(con = con,
                                          unit_level = ifelse(isTRUE(unit_level >= 0), unit_level, 3),
...
```{r}
 ...
 if (nrow(df_copub_countries) > 0) {
   ...
   header <- eval(parse(text = bibliomatrix:::getheader(names(df_copub_countries))))
...
```{r}
 ...
 if (nrow(df_copub_orgs) > 0) {
   ...
   header <- eval(parse(text = bibliomatrix:::getheader(names(df_copub_orgs))))
