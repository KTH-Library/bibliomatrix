...
if (params$use_gauge == TRUE) {
  sector <- gaugeSectors(colors = colors_vb[1])
  gauge(
    value = total_pubs,
    sectors = sector,
    min = 0,
...
```{r}
# TODO: be more defensive here (what if no rows are returned?)
topten <- as.numeric(df_cf[nrow(df_cf) - 1, "top10_share"]) %>% percent()
...
if (params$use_gauge == TRUE) {
  ...
  sector <- gaugeSectors(
    ...
    warning = c(5, 10),
    danger = c(0, 5),
    colors = c(colors_vb[1], colors_vb[2], colors_vb[3])
  )
...
Please register publications in DiVA as soon as possible for these to be included in the Annual Bibliometric Monitoring.

<a href="https://kth.diva-portal.org/dream" class="btn btn-sm btn-primary">Edit your publication data in DiVa</a>
...
# create long format to better fit ggplot
...
df_copub_long <- 
  df_copub %>% 
  gather("indicator", "value", -interval) %>% 
