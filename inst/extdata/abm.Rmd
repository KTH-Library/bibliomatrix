...
if(isTRUE(unit_level >= 0)){
  ...
  unit_label <- kth_displayname(user = params$unit_code, "kthid")
}
...
if(isTRUE(unit_level >= 0)){
  ...
  glue("<p><span title='Legend: 75% or above is high, 60% or above is moderate while lower than 60% is low'>Overall Web of Science coverage for peer reviewed publications in this unit is ",
       "<b>{coveragetext(woscov)}, {100*round(woscov, 3)}%</b>.</span></p>")
...
if (nrow(df_diva) > 0) {
  p <- (abm_graph_diva(df_diva) +
    ...
    abm_plotlyfy(tooltip = c("y", "x", "fill")) |> 
    style(hovertemplate = paste0("<b>%{data.legendgroup}</b><br>",
                                 "<b>Publication year:</b> %{x}<br>",
                                 "<b>Publication volume, frac:</b> %{y:.5r}<extra></extra>")) |>
    layout(
      autosize = TRUE,
      showlegend = TRUE,
  ...
  #plotly::plot_ly(hovertemplate = paste(
     # "%{yaxis.title.text}: %{y:$,.0f}<br>",
     # "%{xaxis.title.text}: %{x%}<br>"
     # ))) 
  
    #layout(margin = list(l = 30, t = 40, r = 30)) |> 
    #plotly:::add_trace(
    # hovertemplate = paste('<br><b>X</b>: %{x}<br>')) 
   # plotly::plot_ly(hovertemplate = paste(
   #   "%{yaxis.title.text}: %{y:$,.0f}<br>",
...
if (nrow(df_diva) > 0) {
  p <- (abm_graph_diva(df_diva) +
    theme(axis.title.x = element_blank())) |> 
    abm_plotlyfy(tooltip = c("y","fill", "x")) |> 
...
if (nrow(df_diva %>% left_join(get_pubtype_order(), by = c("Publication_Type_DiVA" = "diva_publication_type")) %>% filter(WoS_coverage != 0)) > 0) {
  p <- abm_graph_wos_coverage(df_diva) |> 
    abm_plotlyfy(tooltip = c("y", "fill")) |>
    style(hovertemplate = paste0("<b>%{y}</b><br>",
                                 "<b>WoS coverage:</b> %{x:.1%}<extra></extra>"))
  p
} else {
...
if(isTRUE(unit_level >= 0)){
  ...
  years <- df_cit3y %>% filter(substr(Publication_Year, 1, 1) == "2") %>% pull(Publication_Year) %>% as.integer()
...
if (df_oa %>% filter(!is.na(P_tot)) %>% nrow() > 0) {
  abm_graph_oadata_stackedarea(df_oa) |>
    ...
    style(hovertemplate = paste0("<b>%{data.legendgroup}</b><br>",
          "<b>Publication year:</b> %{x}<br>",
          "<b>Nr publications:</b> %{y}<extra></extra>"))
...
if(isTRUE(unit_level >= 0)){
  years <- df_scop_cit %>% filter(substr(Publication_Year, 1, 1) == "2") %>% pull(Publication_Year) %>% as.integer()
