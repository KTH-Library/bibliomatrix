...
if (params$is_employee == TRUE) {
  out <- '<a href="https://kth.diva-portal.org/dream" class="btn btn-sm btn-primary" target="_blank">Edit your publication data in DiVA</a><p>Please register publications in DiVA as soon as possible for these to be included in the Annual Bibliometric Monitoring.</p>'
  cat(out)
}
...
if (nrow(df_diva) > 0) {
  ...
  header <- eval(parse(text = getheader(names(df_diva))))
  ...
  DT::datatable(df_diva,
    ...
    options = list(
#      columnDefs = list(list(className = 'dt-right', targets = c(1:length(names(df_diva)) - 1))),
  ...
  shiny::HTML("<p><i>There are no publications available for this table</i></p>")
}
...
### **Citations 3-year window** {.no-mobile}
...
if (nrow(df_cit3y) > 0) {
  ...
  header <- eval(parse(text = getheader(names(df_cit3y))))

  DT::datatable(df_cit3y,
    container = header,
    caption = htmltools::tags$caption(
      style = 'caption-side: top; text-align: left;',
        'Citations 3-year window, fractional counts. ', 
        htmltools::em('This table is based on Web of Science publication types Article, Proceedings paper, Review, Letter and Editorial.')),
    rownames = FALSE,
    extensions = "Buttons",
    options = list(
      ordering = FALSE,
...
### **Field normalized citations** {.no-mobile}
<style type="text/css">
div.dt-buttons {
  position:relative;
  float:right;
  ...
  DT::datatable(df_cf,
    ...
    caption = htmltools::tags$caption(
      style = 'caption-side: top; text-align: left;',
        ...
        htmltools::em('This table is based on Web of Science publication types Article and Review.')),    
    rownames = FALSE,
    extensions = "Buttons",
    options = list(
      ordering = FALSE,
...
### **Average field normalized citations (Cf)** {.no-mobile}
...
```{r, echo=FALSE, results="asis"}
cat("The table showing data for citations using 3-year windows and fractional counts is based on Web of Science publication types Article, Proceedings paper, Review, Letter and Editorial.<br>")
...
```{r, echo=FALSE, results="asis"}
cat("The table showing data for field normalized citations uses fractionalized 3-year moving average publication counts and is based on Web of Science publication types Article and Review.<br>")
...
### **Citations 3-year window** {.mobile}
...
if (nrow(df_cit3y) > 0) {
  df_cit3y %>% 
    ...
    kable(caption = "Citations 3-year window, fractional counts", col.names = getcolnames(names(df_cit3y)),
...
### **Field normalized citations** {.no-padding .mobile}
...
if (nrow(df_cf) > 0) {
  df_cf %>% 
    ...
    kable(caption = "Field normalized citations - Fractionalized (3-year moving average)", col.names = getcolnames(names(df_cf)),
...
### **Average field normalized citations (Cf)** {.mobile}
...
### **Field normalized journal indicator (JCf)** {.no-mobile}
...
### **Share Journal Top 20%** {.no-mobile}
...
### **Field normalized journal indicator (JCf)** {.mobile}
...
### **Co&#8209;publication over time** {.no-mobile}
...
```{r, echo=FALSE, results="asis"}
cat("The data is based on Web of Science publication types Article and Review.<br>")
...
### **Co&#8209;publications** {.no-padding .mobile}

```{r, fig.width=3.75}
if (nrow(df_copub) > 0) {
  df_copub %>% 
    mutate_at(vars(4, 6), function(x) sprintf("%.1f%%", x * 100)) %>%
    kable(caption = "International and Swedish non-university co-publications (full counts)", col.names = getcolnames(names(df_copub)),
...
### **Co&#8209;publication over time** {.mobile}
```{r, fig.width=6, fig.height=4}
if(df_copub %>% filter(!is.na(P_full)) %>% nrow() > 0) {
  abm_graph_copub(df_copub)
} else {
...
### **Overview by OA type** {.no-mobile}
```{r, fig.width=4, fig.height=4}
if(df_oa %>% filter(!is.na(P_tot)) %>% nrow() > 0) {
  abm_graph_oadata_pie(df_oa)
} else {
  shiny::HTML("<p><i>There are no publications available for this graph</i></p>")
}
```

### **OA publications over time, by type** {.no-mobile}
...
### **Overview by OA type** {.no-padding .mobile}

Overview of Open Access publications, by OA type share
...
### **OA publications over time, by type** {.no-padding .mobile}
```{r, fig.width=5}
if(df_oa %>% filter(!is.na(P_tot)) %>% nrow() > 0) {
  abm_graph_oadata_stackedarea(df_oa)
} else {
