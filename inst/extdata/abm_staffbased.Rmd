...
unit_members <- abm_researchers(params$unit_code)#unit_staff(unit_slug=unit_code)
unit_publ<- abm_staff_data(kthids=unit_members)
  ...
  df_copub <- abm_table5_alt(con = con, data=unit_publ)
  df_woscov <- abm_woscoverage_alt(con = con, data=unit_publ)
  df_oa <- abm_table6_alt(con = con, data=unit_publ)
...
if (isTRUE(unit_level >= 0 & nrow(df_woscov) > 0)) {
...
researchers <- 
  ...
  distinct() %>%
  select(Username = username, Title = `title.en`) %>% collect()
...
if (nrow(df_diva) > 0 && any(df_diva$WoS_coverage > 0)) {
...
if (isTRUE(unit_level >= 0) & nrow(df_woscov)) {
