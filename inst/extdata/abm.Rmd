...
if (nrow(df_diva) > 0) {
  df %>% 
    ...
    kable(align = c("l", rep("r", ncol(df) - 1))) %>% 
...
df <- df_cit3y %>% rename(
  ...
  "Uncited Publications" = "P_uncited",
  "Share Uncited" = "Share_uncited"
)
...
### **Open Access publications, full counts** {.no-padding}

```{r}
df <- df_oa %>% rename(
  "Publication years" = "Publication_Year_ch",
  "Publications" = "P_tot",
  "Open Access publications" = "oa_count",
  "OA gold" = "gold_count",
  "OA hybrid" = "hybrid_count",
  "OA green" = "green_count",
  "OA bronze" = "bronze_count",
  "Not OA" = "closed_count",
  "Share OA" = "oa_share")
...
**Note**: The figures shown in the table are limited to peer-reviewed publications for which Unpaywall could determine the OA type. Unpaywall's method is empirical and does not strictly equate to copyright license. As a consequence, the OA status of a given publication may vary with time --- for example turning from *"Not OA"* to *"Green"* when a full text is added to DiVA.
...
### **Evolution of Open Access publication types**
```{r, fig.width=6, fig.height=4}
if(df_oa %>% filter(!is.na(P_tot)) %>% nrow() > 0) {
...
### **Background / overview (?)** {.no-padding}

### **Indicator glossary** {.no-padding}

### **Contact info and metadata for results(?)** {.no-padding}