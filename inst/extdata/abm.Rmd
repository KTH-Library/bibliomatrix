...
if (has_rows == TRUE) {
  ...
  pub_bullet1 <- abm_bullet_plotly(label = "Field normalized citations (Cf)", 
  ...
  pub_bullet2 <- abm_bullet_plotly(label = "Share Top10% publications", 
...
if (has_rows == TRUE) {
  ...
  jour_bullet1 <- abm_bullet_plotly(label = "Field normalized journal citations (JCf)", 
    value = jcf, reference = 1.0, roundto = 2)

  top20_share <- as.numeric(filter(df_jcf, interval == last_interval)$top20_share)
  
  jour_bullet2 <- abm_bullet_plotly(label = "Share in Top20% journals", value = top20_share, 
...
if (nrow(df_diva) > 0) {
  p <- abm_graph_diva_plotly(df_diva)
  p
} else {
  shiny::HTML("<p><i>There are no publications available for this graph</i></p>")
}
...
if(df_copub %>% filter(!is.na(P_full)) %>% nrow() > 0) {
  abm_graph_copub_plotly(df_copub)
} else {
  shiny::HTML("<p><i>There are no publications available for this graph</i></p>")
}
```
...
if (df_oa %>% filter(!is.na(P_tot)) %>% nrow() > 0) {
  abm_graph_oadata_share(df_oa, type = "plotly")
} else {
  shiny::HTML("<p><i>There are no publications available for this graph</i></p>")
}
```
...
### **Evolution of the number of publications, by OA type**

```{r, fig.width=8, fig.height=8}
if (df_oa %>% filter(!is.na(P_tot)) %>% nrow() > 0) {
  abm_graph_oadata_linegraphs(df_oa, type = "plotly")
} else {
  shiny::HTML("<p><i>There are no publications available for this graph</i></p>")
}
```
