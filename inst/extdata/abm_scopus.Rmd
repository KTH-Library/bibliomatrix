...
# 
# # Retrieve fake scopus data
# db <- con_bib("sqlite")
# scopusdata <- db %>%
#   tbl("masterfile_fejk") %>%
#   collect() %>% 
#   filter(Unit_code == uc &
#            Publication_Year >= abm_config()$start_year &
#            Publication_Year <= abm_config()$stop_year) %>% 
#   select(PID, Publication_Year, Publication_Type_DiVA, Unit_Fraction, Unit_Fraction_adj, scopus_id, scop_jcf, scop_jtop20, scop_ptop10, scop_cf, scop_nuniv, scop_int)
# DBI::dbDisconnect(db)
