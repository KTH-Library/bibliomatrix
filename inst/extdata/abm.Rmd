...
if (params$use_package_data != TRUE) {
  ...
  df_oa <- abm_oa_data(con = con, unit_code = params$unit_code)
...
### **Open Access publications, full counts** {.no-padding .no-mobile}
...
### **Overview of Open Access publications, by OA type share (3D)**
```{r, fig.width=6, fig.height=4}
if(df_oa %>% filter(!is.na(P_tot)) %>% nrow() > 0) {
  abm_graph_oadata_pie3D(df_oa)
}
```

Row
-------
### **Evolution of Open Access publication types (stacked area)**
```{r, fig.width=6, fig.height=4}
if(df_oa %>% filter(!is.na(P_tot)) %>% nrow() > 0) {
...
### **Evolution of Open Access publication types (line chart)**
```{r, fig.width=6, fig.height=4}
if(df_oa %>% filter(!is.na(P_tot)) %>% nrow() > 0) {
  abm_graph_oadata_types_linechart(df_oa)
}
```
