...
if (nrow(df_diva) > 0){
  coverage <-
    df_diva %>%
    filter(Publication_Type_DiVA %in% c("Article, peer review", "Conference paper, peer review")) %>% 
    summarise(totcov = sum(P_frac * WoS_coverage) / sum(P_frac),
              wospubs = sum(P_frac * WoS_coverage))
  totcov <- pull(coverage, totcov)
  wospubs <- pull(coverage, wospubs)
  valueBox(
...
if (params$is_employee == TRUE){
  ...
  wospubtext <- glue("<p>Citation based indicators are based on {round(wospubs, 1)} publications.")
  if(wospubs < 50)
    wospubtext <- paste(wospubtext, " <b>Results for publication volumes below 50 are unreliable.</b>")
  wospubtext <- paste(wospubtext, "</p>")
  cat(wospubtext)
  
  coveragetext <- glue::glue("<p>Web of Science coverage for peer reviewed publications is <b>{100*round(totcov, 3)}%</b>.")
  if(totcov >= 0.75)
    coveragetext <- paste(coveragetext, "Coverage of 75% or above is considered <b<>adequate</b>.")
  if(totcov < 0.75 & totcov >= 0.6)
    coveragetext <- paste(coveragetext, "Coverage between 60% and 75% is considered <b>questionable</b>.")
  if(totcov < 0.6)
    coveragetext <- paste(coveragetext, "Coverage below 60% is considered <b>unreliable</b>.")
  coveragetext <- paste(coveragetext, "</p>")
  cat(coveragetext)
}
```
