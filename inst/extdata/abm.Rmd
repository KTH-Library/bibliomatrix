...
df_diva <- abm_table1(unit_code = demo_unit) 
df_cit3y <- abm_table2(unit_code= demo_unit)
df_cf <- abm_table3(unit_code= demo_unit)
df_jcf <- abm_table4(unit_code= demo_unit)
df_copub <- abm_table5(unit_code= demo_unit) 

#dfs<- abm_data(unit_code = demo_unit) %>% collect()
```
...
```{r}
total_pubs <- 
  df_diva %>% select("2018") %>% sum %>% round(digits=1)
...
publications_kth <- abm_publications(unit_code = demo_unit)
