...
```{r, fig.width=10}
if (nrow(df_diva) > 0) {
  p <- abm_graph_diva(df_diva) |> ggplotly(tooltip = c("y")) |>  # tooltip = c("fill","y")) 
    config(displayModeBar = FALSE) |>
    layout(autosize = TRUE, showlegend = TRUE, margin = list(l = 30, t = 40, r = 30))
  p
} else {
...
```{r, fig.width=7, fig.height=4}
if (df_cf %>% filter(!is.na(pubs_frac)) %>% nrow() > 0){
  time_graph(df_cf, indicator = "top10_share", ma = TRUE, weight = "pubs_frac", refline = .1, percent = TRUE, ylabel = "Share Top 10%")
} else {
  shiny::HTML("<p><i>There are no publications available for this graph</i></p>")
}
```
