...
df %>% 
  mutate_at(vars(starts_with("201"), starts_with("Total")), round) %>%
  mutate_at(vars(starts_with("WoS")), function(x) sprintf("%3.0f%%", x * 100)) %>%
  kable() %>% 
...
df %>% 
  mutate_at(vars(1:3), function(x) sprintf("%.1f", x)) %>%
  kable() %>% 
  kable_styling(bootstrap_options = c("responsive")) %>%
  scroll_box(width = "720px")
