...
if(df_scop_normcit %>% filter(!is.na(P_frac)) %>% nrow() > 0) {
  time_graph(df_scop_normcit, "fwci_x", ma = TRUE, weight = "P_frac", ylabel = "Average FWCI", refline = 1)
} else {
  shiny::HTML("<p><i>There are no publications available for this graph</i></p>")
}
```
...
```{r, fig.height = 4, fig.width = 6}
if(df_scop_normcit %>% filter(!is.na(P_frac)) %>% nrow() > 0) {
  time_graph(df_scop_normcit, "top10_share", ma = TRUE, weight = "P_frac", ylabel = "Share Top 10%", refline = .1, percent = TRUE)
} else {
  shiny::HTML("<p><i>There are no publications available for this graph</i></p>")
}
```
...
if(df_scop_snip %>% filter(!is.na(P_frac)) %>% nrow() > 0) {
  time_graph(df_scop_snip, "avg_snip", ma = TRUE, weight = "P_frac", ylabel = "Average SNIP", refline = 1)
} else {
  shiny::HTML("<p><i>There are no publications available for this graph</i></p>")
}
```  
...
```{r, fig.height = 4, fig.width = 6}
if(df_scop_snip %>% filter(!is.na(P_frac)) %>% nrow() > 0) {
  time_graph(df_scop_snip, "top20_share", ma = TRUE, weight = "P_frac", ylabel = "Share Top 20%", refline = .2, percent = TRUE)
} else {
  shiny::HTML("<p><i>There are no publications available for this graph</i></p>")
}
```  
