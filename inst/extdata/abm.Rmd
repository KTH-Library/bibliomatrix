...
### **Field normalized journal impact (JCf)** <br> `r paste0('*Yearly (<span style="color:', colors_vb['blue'],'">&#8226;&#8226;</span>) and moving average (<span style="color:', colors_vb['darkred'],'">&mdash;</span>)*')`
...
```{r, fig.height = 4, fig.width = 6}
if(df_scop_snip %>% filter(!is.na(P_frac)) %>% nrow() > 0) {
  time_graph(df_scop_snip, "top20_share", ma = TRUE, weight = "P_frac", ylabel = "Share Top 10%", refline = .2, percent = TRUE)
} else {
  shiny::HTML("<p><i>There are no publications available for this graph</i></p>")
}
```  
