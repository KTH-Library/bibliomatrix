...
### Total publications (fractional counts){.value-box}
```{r, fig.width = 2}
if(nrow(df_diva) > 0){
  lastyear <- nth(names(df_diva), -3)
  total_pubs <- sum(df_diva[, lastyear], na.rm = TRUE)
  valueBox(caption = paste("DiVA publications (fractional count)", lastyear),
           value = round(total_pubs, 1),
           color = colors_vb[1],
           href = "#diva_publications")
} else {print("No data")}
```
...
Publications in DiVA {#diva_publications}
=====================================
